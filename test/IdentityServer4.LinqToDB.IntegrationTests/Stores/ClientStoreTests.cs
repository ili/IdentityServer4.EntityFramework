...
namespace IdentityServer4.LinqToDB.IntegrationTests.Stores
{
	public class ClientStoreTests : IClassFixture<DatabaseProviderFixture>
	{
		...
		public async void ComplexTest()
		{
			...
			foreach (var client in Clients.Get())
			{
				var selected = await store.FindClientByIdAsync(client.ClientId);
				Assert.NotNull(selected);

				Assert.Equal(client.ClientName, selected.ClientName);
				Assert.Equal(client.AllowedCorsOrigins, selected.AllowedCorsOrigins);
				Assert.Equal(client.AllowedGrantTypes, selected.AllowedGrantTypes);
				Assert.Equal(client.AllowedScopes, selected.AllowedScopes);
				Assert.Equal(client.Claims.Select(_ => _.Value), selected.Claims.Select(_ => _.Value));
				Assert.Equal(client.ClientSecrets.Select(_ => _.Value), selected.ClientSecrets.Select(_ => _.Value));
				Assert.Equal(client.IdentityProviderRestrictions, selected.IdentityProviderRestrictions);
				Assert.Equal(client.PostLogoutRedirectUris, selected.PostLogoutRedirectUris);
				Assert.Equal(client.RedirectUris, selected.RedirectUris);
			}
